...
네, 시험 문제에서 **아주 명확하게 알려줍니다.** 하지만 "확률을 제출하세요"라고 대놓고 적혀있기도 하고, **"평가지표"를 통해 눈치껏 알아내야 하는 경우**도 많습니다.

시험장에서 **확률 vs 값**을 구분하는 **2가지 절대 기준**을 정리해 드립니다.

---

### 1. 가장 확실한 신호: **"평가 지표 (Metric)"**

문제지 맨 앞에 적혀 있는 **[평가 지표]**를 보면 100% 알 수 있습니다.

| 평가 지표 (Metric) | 제출해야 할 값 | 함수 (Code) | 비고 |
| :--- | :--- | :--- | :--- |
| **ROC-AUC** | **확률 (Probability)** | `predict_proba(X)[:, 1]` | **가장 많이 나옴!** (0.xxx 소수점) |
| **F1-Score** | **분류값 (Class)** | `predict(X)` | (0 또는 1) |
| **Accuracy (정확도)** | **분류값 (Class)** | `predict(X)` | (0 또는 1) |
| **Precision (정밀도)** | **분류값 (Class)** | `predict(X)` | (0 또는 1) |
| **Recall (재현율)** | **분류값 (Class)** | `predict(X)` | (0 또는 1) |
| **RMSE, MAE, R2** | **회귀값 (Value)** | `predict(X)` | (집값, 판매량 등 숫자) |

> **🔥 요약 공식:**
> * **"AUC"**라는 글자가 보이면 👉 **무조건 확률 (`proba`)**
> * **나머지 모든 경우** 👉 **무조건 값 (`predict`)**

---

### 2. 두 번째 신호: **"문제 지문 (Text)"**

문제 설명 텍스트에 힌트가 숨어 있습니다.

* **확률 제출일 때:**
    * "이탈할 **확률(probability)**을 예측하여 제출하시오."
    * "`pred` 칼럼에는 남성일 **확률**을 저장하시오."
    * "0과 1 사이의 **소수점**으로 제출하시오."

* **분류값(0/1) 제출일 때:**
    * "이탈 여부(**0 또는 1**)를 예측하여 제출하시오."
    * "고객의 **등급(0, 1, 2)**을 예측하시오."
    * "분류 **결과(label)**를 제출하시오."
...
시험 문제를 받자마자 **평가 지표(Metric)**부터 동그라미 치세요.

1.  **`ROC-AUC`**다?
    * "아, **`model.predict_proba(test)[:, 1]`** 써야지!" 하고 머릿속에 박제.
2.  **`F1-Score`**나 **`Accuracy`**다?
    * "아, 그냥 **`model.predict(test)`** 써야지!" 하고 박제.

이것만 확인하면 실수할 일 절대 없습니다! 안심하세요. 👍


\*\*`unstack()`\*\*은 \*\*"인덱스(Index)를 컬럼(Column)으로 들어 올려서 옆으로 펼치는 함수"\*\*입니다.

사용자님 말씀대로, 상황에 따라 `melt`나 `pivot_table`보다 훨씬 직관적이고 간단하게 표 모양을 바꿀 수 있습니다. 특히 **`groupby`와 찰떡궁합**입니다.

-----

### 1\. 🖼️ 그림으로 이해하기

\*\*"쌓여 있는(Stack) 인덱스를 옆으로 무너뜨린다(Unstack)"\*\*라고 생각하면 됩니다.

  * **`stack()`**: 컬럼을 인덱스로 내림 (길어짐, Long format)
  * **`unstack()`**: 인덱스를 컬럼으로 올림 (넓어짐, Wide format)

-----

### 2\. 💻 실전 코드 예시 (가장 많이 쓰는 패턴)

시험에서 `unstack()`은 99% 확률로 **`groupby` 다음에** 씁니다.

**상황:** "날짜별, 제품별 판매량을 표로 만들어라."
...
# 1. Groupby 결과 (MultiIndex Series)
# 인덱스가 [날짜, 제품] 두 개로 쌓여 있습니다.
grouped = df.groupby(['날짜', '제품'])['판매'].sum()
print(grouped)
# 날짜  제품
# 1일   A     10
#       B     20
...
### 3\. `melt` vs `pivot` vs `unstack` (시험용 구분)

이 세 가지가 헷갈리는데, \*\*"출발점이 어디냐"\*\*만 보면 됩니다.

| 함수 | 방향 | 출발점 | 설명 |
| :--- | :--- | :--- | :--- |
| **`melt`** | **Wide $\to$ Long** (좁아짐) | **Column** | 가로로 긴 데이터를 세로로 녹일 때 |
| **`pivot`** | **Long $\to$ Wide** (넓어짐) | **Column** | 세로 데이터를 가로 표로 만들 때 (`index`, `columns`, `values` 지정) |
| **`unstack`**| **Long $\to$ Wide** (넓어짐) | **Index** ⭐ | \*\*이미 인덱스가 설정된 상태(Groupby 결과 등)\*\*를 펼칠 때 |

> **💡 결론:**
>
>   * `groupby`를 했다면? ➡ \*\*`unstack()`\*\*이 가장 빠릅니다.
>   * 그냥 쌩 데이터(`df`) 상태라면? ➡ \*\*`pivot_table()`\*\*이 편합니다.

### 4\. 꿀팁 옵션: `fill_value`

`unstack()`을 했는데 빈칸(`NaN`)이 생겼다면? (예: 1일엔 A를 팔았는데 B는 안 판 경우)

```python
# 빈칸을 0으로 채우면서 펼치기
df.groupby(...).sum().unstack(fill_value=0)
```

이것만 알면 `groupby` 후 데이터를 예쁘게 펴는 건 식은 죽 먹기입니다\!