...
빅데이터 분석기사 실기 시험에서 \*\*`np.where`\*\*는 **"조건에 따라 값을 바꿀 때"** 가장 빠르고 자주 쓰이는 필수 함수입니다.

엑셀의 **`=IF()`** 함수와 똑같다고 생각하시면 됩니다.

-----

# ⚡️ `np.where` 완벽 정리 (시험용)

### 1\. 기본 개념

  * **역할:** 조건이 \*\*참(True)\*\*이면 A값을, \*\*거짓(False)\*\*이면 B값을 반환합니다.
  * **장점:** `apply`나 `for`문보다 **속도가 훨씬 빠릅니다.** (대용량 데이터 처리에 필수)
  * **필수:** `import numpy as np`가 필요합니다.
...
"점수가 60점 이상이면 'Pass', 아니면 'Fail'로 표시하시오."

```python
import pandas as pd
import numpy as np

df = pd.DataFrame({'Score': [80, 50, 90, 40]})
...
#### ② 다중 조건 (중첩 사용 - `if` \~ `elif` \~ `else`)

"점수가 90 이상 'A', 80 이상 'B', 나머지는 'C'로 표시하시오."
👉 `np.where` 안에 또 `np.where`를 넣으면 됩니다.

```python
# np.where(조건1, 'A', np.where(조건2, 'B', 'C'))

df['Grade'] = np.where(df['Score'] >= 90, 'A', 
                       np.where(df['Score'] >= 80, 'B', 'C'))
```

#### ③ 값 유지하기 (조건에 안 맞으면 원래 값 그대로)

"음수인 값만 0으로 바꾸고, 양수는 **원래 값 그대로** 두시오."

```python
df = pd.DataFrame({'Value': [10, -5, 20, -1]})

# 거짓일 때(양수일 때) df['Value']를 그대로 씀
...
### 4\. `np.where` vs `apply` (시험 꿀팁)

| 구분 | **`np.where`** | **`apply` (lambda)** |
| :--- | :--- | :--- |
| **속도** | **매우 빠름** (C언어 기반) | 느림 (한 줄씩 처리) |
| **복잡도** | 단순한 if-else에 적합 | 복잡한 로직(함수)에 적합 |
| **추천** | **단순 조건 파생변수 생성 시 무조건 이거\!** | 조건이 너무 복잡할 때만 사용 |
...
> **"조건에 따라 값을 바꾸고 싶다?"**
> ➡️ 고민하지 말고 **`np.where(조건, True값, False값)`** 쓰세요\!

파이썬에서 문자열 내의 **단어 개수**를 세는 가장 표준적인 방법은 `split()` 함수와 `len()` 함수를 조합하는 것입니다.

상황에 따라 두 가지 방법이 있습니다.

-----

### 1\. 기본 파이썬 문자열일 때 (가장 기초)

`split()`은 공백(스페이스, 탭, 엔터 등)을 기준으로 문자열을 쪼개서 **리스트**로 만듭니다. 이 리스트의 길이(`len`)를 재면 단어 개수가 됩니다.

```python
text = "안녕하세요 반갑습니다 파이썬 공부 중입니다"

# 1. 공백 기준으로 쪼개기
words = text.split()
print(words) 
# 결과: ['안녕하세요', '반갑습니다', '파이썬', '공부', '중입니다']
...
> **팁:** `text.split()` 처럼 괄호 안에 아무것도 안 넣어야 띄어쓰기가 여러 번 되어 있어도 깔끔하게 단어만 걸러냅니다. (`text.split(" ")`보다 안전합니다.)

-----

### 2\. 판다스 데이터프레임일 때 (빅분기 실기용)

데이터프레임의 특정 컬럼(Series)에 있는 텍스트들의 단어 수를 일괄적으로 세어서 새로운 컬럼을 만들 때 사용합니다. **`.str` 접근자**를 사용해야 합니다.
...
# 'review' 컬럼의 문장을 쪼개고(split) -> 그 길이(len)를 잰다
...
```
              review  word_count
0         음식이 정말 맛있어요           3
1  서비스가 별로네요 다시는 안 갈래요           5
2                 최고           1
```

**빅데이터 분석기사 실기**에서는 주로 **2번 방법**을 활용하여 텍스트 길이 파생변수를 만들 때 자주 사용됩니다.

네, 아주 중요한 질문입니다\! \*\*"내가 지금 다루는 게 시리즈(Series)냐 데이터프레임(DataFrame)이냐"\*\*에 따라 뽑는 법이 완전히 다릅니다.

시험장에서 가장 많이 헷갈리는 부분이니 딱 정리해 드릴게요.

-----

### 1\. 시리즈 (Series) 일 때

> **특징:** 컬럼이 하나밖에 없음. (인덱스 + 값)
> **상황:** `groupby()['컬럼'].sum()` 처럼 컬럼 하나만 콕 집어서 계산했을 때.

```python
# q2는 시리즈입니다.
# 인덱스(Index): 국가이름 (Venezuela...)
# 값(Value): 맥주소비량 (숫자)

# 1. 이름(국가)이 필요할 때 -> .index 사용
print(q2.index[4])  # "Venezuela" (O)

# 2. 값(숫자)이 필요할 때 -> .iloc 사용
print(q2.iloc[4])   # 158136 (O)
```

### 2\. 데이터프레임 (DataFrame) 일 때

> **특징:** 컬럼이 여러 개이거나, 표 형태를 유지하고 있음.
> **상황:** `sort_values()`를 한 원본 데이터(`df`) 자체일 때.

```python
# df는 데이터프레임입니다. (국가 컬럼이 따로 있음)

# 1. 이름(국가)이 필요할 때 -> .iloc 사용 후 컬럼명 지정
print(df.iloc[4]['국가'])  # (O) "5번째 행의 '국가' 컬럼을 줘"

# 2. .index를 쓰면? -> 행 번호(0, 1, 2...)가 나옴
print(df.index[4])  # (X) 그냥 숫자 4가 나옴 (의미 없음)
```
...
| 상황 | 내 데이터의 형태 | 이름(Key) 뽑을 때 | 값(Value) 뽑을 때 |
| :--- | :--- | :--- | :--- |
| **`groupby` 결과** | **시리즈 (Series)** | **`.index[n]`** | `.iloc[n]` |
| **원본 `df` 정렬** | **데이터프레임 (DF)** | **`.iloc[n]['컬럼명']`** | `.iloc[n]['컬럼명']` |

**결론:**

  * `groupby`로 요약된 녀석(`q2`)에서 이름을 찾으려면 👉 **`.index`**
  * 원본 표(`df`)에서 특정 칸을 찾으려면 👉 **`.iloc`**

이것만 기억하면 `IndexError`는 절대 안 납니다\!

빅데이터 분석기사 실기 시험에서 \*\*"날짜 변환"\*\*과 \*\*"문자열 추출"\*\*은 1유형의 킬러 문항입니다. 시험 직전에 보고 바로 쓸 수 있도록 핵심 코드만 정리해 드립니다.

-----

# 📅 1. 날짜/시간 합치기 (숫자가 섞여 있을 때)

**상황:** 날짜는 `20230101`(int), 시간은 `93000`(int, 9시 30분) 처럼 따로 있고, 시간 앞의 **'0'이 빠져있을 때**.

### ⚡️ 암기 공식: `str` → `zfill` → `to_datetime`

```python
# 1. 시간 컬럼을 문자로 바꾸고, 빈 자리를 0으로 채우기 (6자리 맞춤)
# 예: 93000 -> '093000'
df['Time_Str'] = df['Time_Int'].astype(str).str.zfill(6)

# 2. 날짜와 시간을 문자로 합치기
# 예: '20230101' + '093000' -> '20230101093000'
time_str = df['Date_Int'].astype(str) + df['Time_Str']

# 3. datetime 객체로 변환 (포맷 지정 필수!)
df['DateTime'] = pd.to_datetime(time_str, format='%Y%m%d%H%M%S')
```

### 💡 핵심 포인트

  * **`.str.zfill(6)`**: 시간을 다룰 때 가장 중요\! (HHMMSS 6자리를 맞추기 위해 앞에 0을 채움)
  * **`format='%Y%m%d%H%M%S'`**: 연월일시분초 순서를 판다스에게 알려줌.

-----

# 🔡 2. 문자열 정보 추출 (`str.extract`)

**상황:** 주소에서 \*\*'구'\*\*만 뽑거나, 상품명에서 \*\*'[옵션]'\*\*만 뽑아야 할 때.

### ⚡️ 암기 공식: `df['col'].str.extract(r'(패턴)')`

```python
# 예시 데이터: "서울시 마포구 합정동", "경기 성남시 분당구"

# 1. '구'로 끝나는 단어 추출하기
# \S : 공백이 아닌 글자
# +  : 1개 이상 반복
# () : 추출할 대상
df['Gu'] = df['Address'].str.extract(r'(\S+구)')
```
...
| 상황 | 패턴 코드 | 설명 |
| :--- | :--- | :--- |
| **'\~구' 추출** | `r'(\S+구)'` | 공백 앞부터 '구'까지 덩어리 추출 |
| **괄호 안 내용** | `r'\[(.*?)\]'` | `[`와 `]` 사이의 모든 문자 추출 |
| **숫자만 추출** | `r'(\d+)'` | 연속된 숫자만 추출 |
| **연도(4자리)** | `r'(\d{4})'` | 숫자 4개 연속된 것 추출 |

### 💡 핵심 포인트

  * 반드시 **`r'( )'`** 안에 패턴을 넣어야 합니다.
  * \*\*`()` (괄호)\*\*가 없으면 추출되지 않습니다. (괄호 안의 것만 가져오라는 뜻)

-----

### 📝 시험장 요약 노트

1.  시간이 `930` 처럼 잘려 있다면? 👉 **`.astype(str).str.zfill(4)`** (또는 6)
2.  주소에서 특정 단어를 꺼내야 한다면? 👉 **`.str.extract(r'(\S+구)')`**

이 두 가지만 기억하면 전처리 문제 2개는 먹고 들어갑니다\! 합격하세요\! 💪